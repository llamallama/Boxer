...
This file documents the integration of the upstream DOSBox PowerPC dynrec backend (`patch-r4301.diff` against vanilla DOSBox SVN r4301) into Boxer's older `leopard_legacy` DOSBox tree, the divergences that mattered, and the one bug that prevented the JIT from running until it was found.
...
Boxer's existing PPC build had a working DOSBox tree but no recompiler — it ran games using the slow `normal` core. There is an old upstream DOSBox patch, `patch-r4301.diff`, which adds a `risc_ppc.h` backend to `core_dynrec`. Vanilla DOSBox SVN at r4301 plus that patch builds and runs correctly on OS X 10.4 PPC.
...
The PPC dynrec backend (`risc_ppc.h` and the supporting infrastructure changes in `cache.h`, `decoder_basic.h`, `decoder_opcodes.h`, `core_dynrec.cpp`, `mem.h`, and `drive_fat.cpp`) is from the upstream DOSBox community patch known as `patch-r4301.diff`. All credit for the JIT itself goes to its author. This integration glues that patch into Boxer's older, divergent tree.
