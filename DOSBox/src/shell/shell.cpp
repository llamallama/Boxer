...
void DOS_Shell::Run(void) {
	...
	if (cmd->FindString("/INIT",line,true)) {
		...
		//Boxer drops DOSBox's startup banner output; the surrounding Boxer UI handles
...
public:
	AUTOEXEC(Section* configuration):Module_base(configuration) {
		...
		while (control->cmdline->FindCommand(dummy++,line) && !command_found) {
			//--Added by Alun Bestor: detect a physfs source (path with ':' after the optional drive prefix)
			...
			if (test.st_mode & S_IFDIR) {
				...
				*name++ = 0;
				...
				if ((strstr(name,".ZIP") != 0) || (strstr(name,".7Z") != 0)) {
					//--Added by Alun Bestor: PHYSFS archive support
