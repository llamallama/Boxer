...
#define CGA16_READER(OFF) (base[(vidstart +(OFF))& (8*1024 -1)])
  ...
  temp[1] = (CGA16_READER(0) >> 6) & 3;
	...
	Bitu i = 2;
	for (Bitu x=0;x<vga.draw.blocks;x++) {
		//--Modified 2026-05-05 to fix swapped pixel columns on PowerPC Macs
		//Take r4301's improved CGA composite mask but keep Boxer's byteswap.
		//r4301 still emits a packed Bit32u to TempLine here, so big-endian
		//hosts need CFSwapInt32HostToLittle to land bytes in the order the
		//framebuffer expects.
		*draw++ = CFSwapInt32HostToLittle(0xc0708030 | temp[i] | (temp[i+1] << 8) | (temp[i+2] << 16) | (temp[i+3] << 24));
		i += 4;
		*draw++ = CFSwapInt32HostToLittle(0xc0708030 | temp[i] | (temp[i+1] << 8) | (temp[i+2] << 16) | (temp[i+3] << 24));
		i += 4;
